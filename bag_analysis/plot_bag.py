...
def plot(filename, robot_name):
    ...
    for i, bag_message in enumerate(messages):
        ...
        change_robot_frame = np.array([[(radius * msg.vel_right)/2 + (radius * msg.vel_left)/2], 
                                        [0], 
                                        [(radius * msg.vel_right)/baseline + (radius * msg.vel_left)/baseline]])
        theta += change_robot_frame[2]*dt
        
        change_in_xi_world_frame = np.array([[np.cos(theta)[0], -np.sin(theta)[0], 0], 
                                       [np.sin(theta)[0], np.cos(theta)[0], 0], 
                                       [0, 0, 1]]) @ change_robot_frame
        
        xi_world_frame += change_in_xi_world_frame*dt
        print(xi_world_frame[0, 0]*1e+3, xi_world_frame[1, 0]*1e+3)
